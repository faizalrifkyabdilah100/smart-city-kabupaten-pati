...
const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden px-4 sm:px-6 z-10">

      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className={`relative z-10 backdrop-blur-xl border p-5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-md transition-colors duration-500 ${isDark
          ? 'bg-slate-900/60 border-white/10'
          : 'bg-blue-950/45 border-white/20'
        }`}
      >
        ...
        <div className="mt-3 sm:mt-4 text-center">
          <button
            type="button"
            onClick={() => navigate('/')}
            className="text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg transition-all text-blue-100 bg-white/10 hover:bg-white/20 active:bg-white/25"
          >
            Kembali ke Beranda
          </button>
        </div>
